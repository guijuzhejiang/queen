...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, ignore_colors: bool = False):
        ...
        if self.latent_args.f_dc_invert_type == "autoenc" and not isinstance(self.latent_decoders["f_dc"],
                                                                             DecoderIdentity):
            init, decoder_state_dict = init_latents(self.latent_args, fused_color.flatten(1).cuda(), "f_dc",
        ...
        if self.latent_args.rot_invert_type == "autoenc" and not isinstance(self.latent_decoders["rot"],
                                                                            DecoderIdentity):
    ...
    def create_from_depth(self, cameras, spatial_lr_scale, downsample_scale=2, alpha_thresh=0.1, renderFunc=None):
        ...
        for idx in range(len(cameras)):
            ...
            with torch.no_grad():
                world_xyz_colmap = torch.cat((self._xyz, torch.ones_like(self._xyz[:, 0:1])), dim=1)
                cam_xyz_colmap = torch.matmul(camera.world_view_transform.T.unsqueeze(0),
                                              world_xyz_colmap.unsqueeze(-1)).squeeze(-1)
                cam_hom_colmap = torch.matmul(camera.projection_matrix.T.unsqueeze(0),
                                              cam_xyz_colmap.unsqueeze(-1)).squeeze(-1)
                cam_proj_colmap = cam_hom_colmap[:, :3] / cam_hom_colmap[:, 3:]

                in_frustum = torch.logical_and(torch.all(cam_proj_colmap < 1, dim=1),
                                               torch.all(cam_proj_colmap > -1, dim=1))
            ...
            with torch.no_grad():
                ...
                img = F.grid_sample(depth.unsqueeze(0).unsqueeze(0), grid.to(depth).reshape(1, H, W, 2),
                ...
                img_coords = torch.floor(
                    (grid[:cam_proj_colmap_filtered.shape[0]] + 1) * 0.5 * torch.tensor([W, H]).to(grid).unsqueeze(
                        0)).long()
            ...
            with torch.no_grad():
                ...
                world_xyz = torch.matmul(torch.linalg.inv(camera.world_view_transform.T).unsqueeze(0),
                                         cam_xyz.unsqueeze(-1)).squeeze(-1)[:, :3]
                ...
                alpha_mask = (alpha < alpha_thresh).reshape(
                    -1)  # 1 implies we add points from our GT depth, 0 implies neighborhood points already exist from colmap
                if alpha_mask.sum() == 0:
                    continue
        ...
        if isinstance(self.latent_decoders["f_rest"], DecoderIdentity):
            ...
            new_features_rest = torch.zeros((fused_color.size(0), self.latent_decoders["f_rest"].latent_dim)).to(
                fused_color).contiguous()
    ...
    def create_from_depth_immersive(self, cameras, spatial_lr_scale, downsample_scale=2, alpha_thresh=0.1,
                                    renderFunc=None):
        self.spatial_lr_scale = spatial_lr_scale
        ...
        for idx in range(len(cameras)):
            ...
            with torch.no_grad():
                ...
                out = torch.linalg.lstsq(torch.cat((inverse_depths, torch.ones_like(inverse_depths)), dim=1),
                                         inverse_gt_depths)
    ...
    def training_setup(self, training_args):
        ...
        for i, param in enumerate(self.param_names):
            ...
            if not isinstance(self.latent_decoders[param], DecoderIdentity):
                l += [{'params': self.latent_decoders[param].parameters(), 'lr': training_args.ldecs_lr[i],
                       "name": f"ldec_{param}"}]
        ...
        for i, attribute in enumerate(self.param_names):
            if isinstance(self.latent_decoders[attribute], DecoderIdentity):
                if self.prev_atts[attribute] is not None and self.gate_atts is not None and self.gate_params[attribute]:
                    xyz = self.latent_decoders["xyz"](self._latents["xyz"])
                    prev = self.xyz_before[self.mapping.long()]
                    ...
                    ungated_residuals = self.gate_atts(residual_xyz)[ungated_xyz_indices.long()]
                    ...
                    reconstructed_xyz[ungated_xyz_indices.long()] += ungated_residuals
                ...
                if hasattr(self.latent_decoders[attribute], 'decoded_att'):
                    decoder_state_dict[attribute]['decoded_att'] = self.latent_decoders[
                        attribute].decoded_att.detach().cpu()
    ...
    def load_compressed_pkl(self, path):
        ...
        for attribute in latents:
            if isinstance(latents[attribute], dict) and 'num_gaussians' in latents[attribute]:
                if num_gaussians is None:
                    ...
                    raise ValueError(
                        f"Inconsistent number of gaussians in compressed data: {num_gaussians} vs {latents[attribute]['num_gaussians']}")
        ...
        for i, attribute in enumerate(latents):
            if self.latent_args.gate_params[i] == 'on':
                ...
                prev_att_mapping = latents[attribute]["mapping"].cuda().long()
                ...
                reconstructed[ungated_xyz_indices.long()] += ungated_residuals
                ...
                if latent_decoders_dict[attribute] == "LatentDecoder":
                    ...
                    self._latents[attribute] = nn.Parameter(
                        latents[attribute].uncompress().cuda().requires_grad_(False))
                    ...
                    if test_output.shape[-1] != expected_dim:
                        print(
                            f"Warning: {attribute} decoder output shape {test_output.shape} != expected {expected_dim}")
    ...
    def load_ply(self, path, verbose=False):
        ...
        if len(extra_f_names) == expected_extra_f_dim:
            ...
            if verbose:
                print(
                    f"GaussianModel::load_ply(): parsed SH extra f dim ({len(extra_f_names)}) exceeds expectation ({expected_extra_f_dim}).")
        else:
            raise RuntimeError(
                f"GaussianModel::load_ply(): parsed SH extra f dim ({len(extra_f_names)}) does not reach expectation ({expected_extra_f_dim})")
        ...
        self._latents["f_dc"] = nn.Parameter(
            torch.tensor(features_dc, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(
                True))
        self._latents["f_rest"] = nn.Parameter(
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            masks[group['name']].data = torch.cat((masks[group['name']],
                                                   torch.ones(extension_tensor.shape[0], 1).bool().to(
                                                       masks[group['name']].device)), dim=0)

        return optimizable_tensors

    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation, new_flow,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, is_dynamic=False):
        ...
        self.added_mask = torch.cat(
            (self.added_mask, torch.ones(new_xyz.shape[0]).bool().to(self._latents["xyz"].device)))
        ...
        if is_dynamic:
            ...
            if new_mapping.max() >= self.xyz_before.shape[0]:
                new_mapping[new_mapping >= self.xyz_before.shape[0]] = new_mapping[
                    new_mapping[new_mapping >= self.xyz_before.shape[0]]]
    ...
    def densify_dynamic(self, max_grad, min_opacity, extent, max_screen_size):
        ...
        if self.mapping.max() >= self.xyz_before.shape[0]:
            self.mapping[self.mapping >= self.xyz_before.shape[0]] = self.mapping[
                self.mapping[self.mapping >= self.xyz_before.shape[0]]]
