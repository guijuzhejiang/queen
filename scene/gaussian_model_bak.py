        ...
        for i,attribute in enumerate(self.param_names):
            if isinstance(self.latent_decoders[attribute], DecoderIdentity):
                if self.prev_atts[attribute] is not None and self.gate_atts is not None and self.gate_params[attribute]:
                    ...
                    # 2) 确保 prev 是 tensor；若不是则转换（并放到 residual_xyz 的 device 上优先）
                    if not torch.is_tensor(prev):
                        # 如果 residual_xyz 存在，优先使用它的 device；否则用默认 device
                        target_device = None
                        try:
                            target_device = residual_xyz.device
                        except Exception:
                            target_device = None
                        prev = torch.as_tensor(prev, device=target_device)
                    ...
                    # --- begin patch ---
                    # 之前的索引（可能会抛出 IndexError）
                    # ungated_residuals = self.gate_atts(residual_xyz)[ungated_xyz_indices]

                    # 更稳健的替代实现：
                    gate_out = self.gate_atts(residual_xyz)

                    # 确保 ungated_xyz_indices 在同一 device 上（避免 device mismatch）
                    if ungated_xyz_indices.device != gate_out.device:
                        ungated_xyz_indices = ungated_xyz_indices.to(gate_out.device)

                    # 如果是布尔 mask，直接用布尔索引
                    if ungated_xyz_indices.dtype == torch.bool:
                        ungated_residuals = gate_out[ungated_xyz_indices]
                    else:
                        # 不是布尔的情况下，尝试把索引转换为 long（整数索引）
                        # 如果是浮点数（例如从某些计算得到的小数），直接转为 long（下取整）
                        try:
                            idx_long = ungated_xyz_indices.long()
                        except Exception:
                            # 最后兜底：先把 tensor 移到 CPU，再转 long（极少见）
                            print("Warning: Failed to convert ungated_xyz_indices to long. Trying to move to CPU.")
                            idx_long = ungated_xyz_indices.cpu().long().to(gate_out.device)

                        ungated_residuals = gate_out[idx_long]
                    # --- end patch ---
                    ...
                    # 3) 确保 prev 在正确的 device（与后续要修改的张量一致），并且 dtype 与 ungated_residuals 兼容
                    #    如果 ungated_residuals 已经定义，优先匹配其 dtype（通常 float32）
                    if 'ungated_residuals' in locals():
                        try:
                            prev = prev.to(device=ungated_residuals.device, dtype=ungated_residuals.dtype)
                        except Exception:
                            # 兜底：只切 device
                            prev = prev.to(device=ungated_residuals.device)

                    # Verify reconstruction matches _xyz
                    # reconstructed_xyz = prev.clone()
                    # 4) 彻底保证 contiguous，并 clone 为 reconstructed_xyz（clone 应该不会再抛异常）
                    reconstructed_xyz = prev.clone().contiguous()
                    # 5) 规范化索引：把 ungated_xyz_indices 恢复为合法的 long 或 bool 索引，并放到 reconstructed_xyz 相同 device
                    if ungated_xyz_indices is None:
                        # nothing to do
                        pass
                    else:
                        # 如果是布尔掩码，确保长度匹配
                        if isinstance(ungated_xyz_indices, torch.Tensor) and ungated_xyz_indices.dtype == torch.bool:
                            # 如果布尔掩码长度与 reconstructed_xyz 不一致，尝试扩展或报错
                            if ungated_xyz_indices.numel() != reconstructed_xyz.shape[0]:
                                # 如果掩码是一列索引（1D），可能需要 reshape/expand，给出明确错误信息
                                raise ValueError(
                                    f"Bool index length {ungated_xyz_indices.numel()} != reconstructed_xyz length {reconstructed_xyz.shape[0]}")
                            idx_for_use = ungated_xyz_indices.to(reconstructed_xyz.device)
                        else:
                            # 否则把索引转换为 long（先到 CPU 再转 long 可避免某些 dtype 的问题）
                            if not torch.is_tensor(ungated_xyz_indices):
                                idx_tmp = torch.as_tensor(ungated_xyz_indices)
                            else:
                                idx_tmp = ungated_xyz_indices
                            # 转为 long 并放到目标 device
                            idx_for_use = idx_tmp.to(dtype=torch.long, device=reconstructed_xyz.device)

                        # 6) 空索引保护与维度一致性检查（以防 ungated_residuals 长度和索引数量不匹配）
                        if idx_for_use.numel() == 0:
                            # nothing to add
                            pass
                        else:
                            # 确保 ungated_residuals 第一维长度等于索引数量
                            if ungated_residuals.shape[0] != idx_for_use.numel():
                                raise ValueError(
                                    f"The length of ungated_residuals ({ungated_residuals.shape[0]}) != number of indices ({idx_for_use.numel()}).")
                            # 最终的安全索引与加法
                            reconstructed_xyz[idx_for_use] += ungated_residuals
