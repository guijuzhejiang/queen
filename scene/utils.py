...
def load_model_da2(args, device):
    # 1. Construct model
    model_configs = {
    ...
    model = DepthAnythingV2(**model_configs[args.da2_model_encoder]) # pick B/L/S, etc.
    state = torch.load(args.depth_model_ckpt_da2, map_location=device)
    model.load_state_dict(state)
    model.to(device).eval()

    # 2. Create transform function (color, resize, normalize)
    transform = Compose([
        Resize(
            width=args.da2_input_size,
            height=args.da2_input_size,
            resize_target=False,
    ...
    # 3. Resolution DA v2 expects
    net_w, net_h = args.da2_input_size, args.da2_input_size  # example; use the official size or your chosen size
...
def get_depth_model(args: ModelParams):
    ...
    if args.depth_model_type == "midas":
        print("Loading MiDaS model")
        return load_model(device, args.depth_model_ckpt, 'dpt_beit_large_512', optimize=False, height=None, square=False)
    elif args.depth_model_type == "da2":
        print("Loading DA2 model")
        return load_model_da2(args, device)
    else:
        raise ValueError(f"Unknown depth_model_type: {args.depth_model_type}")
